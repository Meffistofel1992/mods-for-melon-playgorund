...
struct LastVisibleScreen {
    static var lastAppearedScreenName: String = "" {
        willSet {
            Logger.info_MMP("\(newValue) was appeared")
        }
    }
}
...
//                })
    ...
    func log<T>(_ message:@autoclosure () -> T, level: LogLevel_MMP, filename: String, line: Int) {
...
//                                                                    self.backgroundTaskIdentifier.rawValue))
            ...
            Logger.verbose_MMP("Ending background task")
        }
    }

    fileprivate func traceMessage(_ msg: String) {
        if SlimConfig.enableConsoleLogging && SlimLogglyConfig.logglyLogLevel == LogLevel_MMP.trace {
            Logger.verbose_MMP(msg)
        }
    }
}
